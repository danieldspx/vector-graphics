...
Toolbox::Toolbox(ivec2 position, int width, int height, std::function<void(IEntity*)> _addEntityCallback, std::function<void()> _changeColorCallback, std::function<void()> _removeEntityCallback, std::function<void(int)> _swapFocusedEntity): pos(position), width(width), height(height) {
    ...
    entities->addEntity(new ActionButton(ivec2{pos.x,pos.y + buttonHeight*7}, width, buttonHeight, saveColor, "Save", sendBackward), 0);
}
